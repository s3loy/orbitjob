...
func Prepare(configPath, imagesPath, runID, runRoot, apiURL, bootstrapKey, profile string) error {
	...
	// The TLS secret is generated rather than committed, so the private key
	// never enters git history.
	if err := generateTLSManifest(); err != nil {
		return fmt.Errorf("generate fixture tls secret: %w", err)
	}
...
// generateTLSManifest regenerates the self-signed fixture certificate via
// deploy/load/make-tls-cert.sh, so the TLS secret the apply loop expects
// always exists and never comes out of git.
func generateTLSManifest() error {
	out, err := exec.Command("bash", "deploy/load/make-tls-cert.sh").CombinedOutput()
	if err != nil {
