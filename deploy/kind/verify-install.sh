...
# The password comes from the manifest just applied, which stays the only
# place the dev-install credential is written down.
postgres_password="$(sed -n 's/^  POSTGRES_PASSWORD: //p' "$root/deploy/kind/postgres-17.yaml")"
printf '%s\n' "postgres://postgres:${postgres_password}@orbitjob-postgres.orbitjob.svc:5432/orbitjob?sslmode=disable" >"$dsn_file"
...
psql_exec() {
  kubectl exec -n "$namespace" deployment/orbitjob-postgres -- env "PGPASSWORD=${postgres_password}" \
