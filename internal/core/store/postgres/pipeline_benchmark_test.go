...
func BenchmarkPipelineClaimComplete(b *testing.B) {
	...
	for b.Loop() {
		...
		_, _ = execUC.RunOnce(context.Background(), "tenant-pipe", "worker-1", 1, 60*time.Second, nil)
	}
}
...
func BenchmarkPipelineFull(b *testing.B) {
	...
	for b.Loop() {
		...
		// Phase 3: Execute
		completed, _ := execUC.RunOnce(context.Background(), "tenant-full", "worker-1", 1, 60*time.Second, nil)
