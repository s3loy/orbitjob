...
// The two job definition gates refuse a scoped caller because a definition has
// no group column: serving one would hand the caller the whole tenant under a
// narrower promise. The refusal itself is already pinned by normalize_test.go;
// these tests prove the guard is load-bearing -- that the RequireUnscoped call
// is the only thing rejecting a scoped request -- by comparing the real gates
// against in-test copies with just that call removed, the mutant of the red
// proof. The 403 mapping of the refusal is verified by inspection of
// internal/admin/http/errors.go, not by a test here.
...
// t3NormalizeJobGetWithoutScopeGuard is NormalizeGetInput with the
// resource.RequireUnscoped call removed and nothing else changed. It is the
// mutant the load-bearing proof runs against.
func t3NormalizeJobGetWithoutScopeGuard(in GetInput) (GetInput, error) {
	if in.ID < 1 {
...
// t3NormalizeJobListWithoutScopeGuard is NormalizeListInput with the
// resource.RequireUnscoped call removed and nothing else changed.
func t3NormalizeJobListWithoutScopeGuard(in ListInput) (ListInput, error) {
	tenantID := strings.TrimSpace(in.TenantID)
	if len(tenantID) != 26 {
...
func TestJobGetScopeGuardIsLoadBearing(t *testing.T) {
	...
	// above no longer proves the guard is what protects the read.
	if _, mutantErr := t3NormalizeJobGetWithoutScopeGuard(scoped); mutantErr != nil {
...
func TestJobListScopeGuardIsLoadBearing(t *testing.T) {
	...
	// being served.
	mutantOut, mutantErr := t3NormalizeJobListWithoutScopeGuard(scoped)
	if mutantErr != nil {
		t.Fatalf("mutant without the guard refused the same request with %v; the refusal does not come from the guard alone", mutantErr)
	}
...
func TestJobQueryTenantContract(t *testing.T) {
	// tenant_id on both definition gates is tenants.id: exactly 26 characters,
	// no slug, no default. The length bounds around this floor are pinned by
