...
| Scheduler runtime (MVP) | 已实现（代码落地，集成测试待收敛） | `cmd/scheduler` + bounded batch tick + misfire policy + 原子调度事务 |
...
Execution plane foundation 的契约与边界见 [docs/execution-plane.md](./docs/execution-plane.md)。
...
| `APP_ENV` | 日志与运行环境标识 |
| `SCHEDULER_BATCH_SIZE` | scheduler 每个 tick 最大处理 job 数，默认 `100` |
| `SCHEDULER_TICK_INTERVAL_SEC` | scheduler tick 周期秒数，默认 `5` |
...
| `cmd/scheduler` | scheduler MVP 入口（批量扫描与调度） |
...
| `internal/core/store/postgres` | job/instance/worker 写侧 repository |
