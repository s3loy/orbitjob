...
CREATE INDEX idx_slis_tenant ON slis(tenant_id);
CREATE INDEX idx_slis_tenant_deleted ON slis(tenant_id, deleted_at) WHERE deleted_at IS NULL;
...
CREATE INDEX idx_slos_tenant_status ON slos(tenant_id, status) WHERE deleted_at IS NULL;
CREATE INDEX idx_slos_sli ON slos(sli_id);
...
CREATE INDEX idx_sli_snapshots_lookup ON sli_snapshots(tenant_id, sli_id, window_start, window_end);
CREATE INDEX idx_sli_snapshots_window ON sli_snapshots(window_start, window_end);
...
CREATE INDEX idx_budgets_lookup ON budgets(tenant_id, slo_id, window_start, window_end);
CREATE INDEX idx_budgets_status ON budgets(tenant_id, status) WHERE status IN ('at_risk', 'exhausted');
...
CREATE INDEX idx_budget_alerts_lookup ON budget_alerts(tenant_id, slo_id, status);
CREATE INDEX idx_budget_alerts_active ON budget_alerts(tenant_id, status) WHERE status = 'active';
